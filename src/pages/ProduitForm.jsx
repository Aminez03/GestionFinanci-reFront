...
import { FaTag, FaMoneyBillWave, FaPercent, FaArrowLeft, FaSave, FaPlus } from "react-icons/fa";
...
const ProduitForm = () => {
  ...
  const [formData, setFormData] = useState({ nom: "", prixHT: "", tvaId: "", description: "" });
  ...
  const validateForm = () => {
    ...
    if (!formData.prixHT || parseFloat(formData.prixHT) <= 0) {
      newErrors.prixHT = "Le prix HT doit être supérieur à 0";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) await updateProduit(id, formData);
      else await createProduit(formData);
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="fw-bold text-primary mb-2">
            ...
            {id ? "Modifier un Produit" : "Nouveau Produit"}
          </h2>
          <p className="text-muted mb-0">
            {id ? "Modifiez les informations de ce produit" : "Ajoutez un nouveau produit à votre catalogue"}
          </p>
        </div>
        <Button
          variant="outline-secondary"
          className="rounded-pill px-4 py-2"
          onClick={() => navigate("/ProduitList")}
        >
      ...
      <Card className="border-0 shadow-sm" style={{ maxWidth: "100%" }}>
        <Card.Header className="bg-primary bg-opacity-10 border-0">
          <h5 className="mb-0 fw-semibold text-primary">
            <i className={`bi ${id ? "bi-box-seam" : "bi-box"} me-2`}></i>
            {id ? "Modification" : "Création"} de Produit
          </h5>
        </Card.Header>
        ...
        <Card.Body className="p-4">
          ...
          <Form onSubmit={handleSubmit}>
            <Row className="g-4">
              ...
              <Col md={6}>
                <Form.Group>
                  <Form.Label className="fw-semibold text-muted">Nom du Produit *</Form.Label>
                  <InputGroup>
              ...
              <Col md={6}>
                <Form.Group>
                  <Form.Label className="fw-semibold text-muted">Prix HT (DT) *</Form.Label>
                  <InputGroup>
                    <InputGroup.Text className="bg-white border-end-0">
                    ...
                    <Form.Control
                      ...
                      onChange={handleChange}
                      placeholder="0.00"
                      className={`border-start-0 shadow-sm ${errors.prixHT ? 'is-invalid' : ''}`}
                      required
                    />
                  </InputGroup>
                  {errors.prixHT && <div className="invalid-feedback d-block">{errors.prixHT}</div>}
                </Form.Group>
              </Col>
