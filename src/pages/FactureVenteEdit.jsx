...
import { getFactureVenteById, updateFactureVente } from "../services/factureVenteService";
...
if (id) {
  ...
  let clientData = {};
  try {
    clientData = await getPersonneMoraleByTiersId(factureData.clientId);
  } catch (error) {
  ...
  setFormData({
    ...
    dateLivraison: factureData.dateLivraison?.split("T")[0] || new Date().toISOString().split("T")[0],
    statut: factureData.statut || "Brouillon",
    timbre: Number(factureData.timbre) || 0.6,
    client: {
      raisonSociale: clientData.raisonSociale || factureData.client?.raisonSociale || "",
      email: clientData.email || factureData.client?.email || "",
      telephone: clientData.telephone || factureData.client?.telephone || "",
      adresse: clientData.adresse || factureData.client?.adresse || "",
      matriculeFiscale: clientData.matriculeFiscale || ""
    }
  });

  // ✅ Lignes services sécurisées
...
const dataToSend = {
      ...
      lignesServices: lignesServices.map(l => ({
        serviceid: l.serviceid,
        duree: l.duree,
        tarifhoraire: l.tarifhoraire, // ✅ ajouté
        remise: 0,
      })),

    };
      await updateFactureVente(id, dataToSend);
      ...
      <Card className="shadow-sm p-4" style={{ maxWidth: "95%" }}>
        <Card.Body>
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h3 className="text-primary"><FaFileInvoiceDollar className="me-2" />Modifier Facture Vente</h3>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Row className="mb-3">
              ...
              <Col md={6}>
                <Form.Group>
                                  <h1 className="mt-1 small">{formData.client.raisonSociale}</h1>
            ...
            <Row className="mb-3">
              <Col md={6}>
                <Form.Group>
                  ...
                  <Form.Control type="date" name="dateFacture" value={formData.dateLivraison} onChange={handleChange} />
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group>
          ...
          {/* --- Modales Clients / Services / Produits --- */}
            ...
            <Modal.Body>
              ...
              <div style={{ maxHeight: "300px", overflowY: "auto" }}>
                {filteredClients.map(client => (
                  <div key={client.id} className="p-2 border-bottom" style={{ cursor: "pointer" }} onClick={() => handleClientSelect(client)}>
...
export default FactureVenteEdit;
