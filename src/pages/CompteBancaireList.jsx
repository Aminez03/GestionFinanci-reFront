...
import { FaTrash, FaUniversity, FaPlus, FaEdit } from "react-icons/fa";
import { 
...
const CompteBancaireList = () => {
  ...
  // --- Fetch Comptes et Factures ---
  const fetchData = async () => {
    setLoading(true);
    try {
      const comptesData = await getAllComptesBancaires();
      setComptes(Array.isArray(comptesData) ? comptesData : comptesData.data || []);
  ...
  const calculateStats = () => {
    ...
    const totalAmount = comptes.reduce((sum, c) => sum + (Number(c.reglement?.montant) || 0), 0);
    const totalBalance = comptes.reduce((sum, c) => sum + (Number(c.solde) || 0), 0);
    const virementCount = comptes.filter(c => c.reglement?.modePaiement === "virement").length;
    const especesCount = comptes.filter(c => c.reglement?.modePaiement === "especes").length;
    const uniqueBanks = [...new Set(comptes.map(c => c.banque).filter(Boolean))].length;
  ...
  const getFilteredAndSortedComptes = () => {
    let filtered = comptes.filter(c => {
      const matchesSearch = !searchTerm || 
        ...
        c.rib?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        c.id.toString().includes(searchTerm);
      
      const matchesBank = !bankFilter || c.banque === bankFilter;
      const matchesMode = !modeFilter || c.reglement?.modePaiement === modeFilter;
      
      return matchesSearch && matchesBank && matchesMode;
    });
  ...
  const filteredComptes = getFilteredAndSortedComptes();
  ...
  const handleOpenModal = (compte = null) => {
    if (compte) {
      setFormData({
        id: compte.id,
        montant: compte.reglement?.montant || "",
        solde: compte.solde || "",
        rib: compte.rib || "",
        banque: compte.banque || "",
modePaiement: compte.reglement?.modePaiement || "virement",
        factureId: compte.reglement?.factureId || ""
      });
    } else {
      setFormData({
        id: null,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await updateCompteBancaire(formData.id, formData);
      } else {
        await createCompteBancaire(formData);
      }
      handleCloseModal();
  ...
  if (loading) return (
    <div className="d-flex flex-column justify-content-center align-items-center mt-5">
      ...
      <p className="mt-3 text-muted fs-5">Chargement des comptes bancaires...</p>
    </div>
  );
    ...
    <div className="content">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Gérez et suivez tous vos règlements bancaires</p>
        </div>
        <Button
          variant="primary"
          size="lg"
          ...
          <Card className="border-0 shadow-sm h-100">
            <Card.Body className="text-center">
              <div className="d-flex align-items-center justify-content-center mb-2">
                ...
                <div>
                  <h3 className="fw-bold text-primary mb-0">{stats.totalComptes}</h3>
            ...
            <Card.Body className="text-center">
              <div className="d-flex align-items-center justify-content-center mb-2">
                <div className="bg-warning bg-opacity-10 rounded-circle p-3 me-3">
                  <i className="bi bi-arrow-left-right text-warning fs-4"></i>
                </div>
                <div>
                  <h3 className="fw-bold text-warning mb-0">{stats.virementCount}</h3>
                  <small className="text-muted">Virements</small>
                </div>
              </div>
            </Card.Body>
          </Card>
          ...
          <Row className="g-3">
            <Col md={3}>
              ...
              <InputGroup>
                ...
                <Form.Control
                  type="text"
                  placeholder="ID, banque, RIB ou facture..."
                  value={searchTerm}
            ...
            <Col md={2}>
              <Form.Label className="fw-semibold text-muted">Banque</Form.Label>
              <Form.Select
                value={bankFilter}
              ...
              >
                ...
                {[...new Set(comptes.map(c => c.banque).filter(Boolean))].map(bank => (
                  <option key={bank} value={bank}>{bank}</option>
                ))}
              </Form.Select>
            </Col>
            <Col md={2}>
              <Form.Label className="fw-semibold text-muted">Mode Paiement</Form.Label>
              <Form.Select
                value={modeFilter}
        ...
        <Card.Header className="bg-white border-0 py-3">
          <div className="d-flex justify-content-between align-items-center">
            <h5 className="mb-0 fw-semibold text-dark">
              ...
              Liste des Règlements Bancaires
            </h5>
            <Badge bg="light" className="text-dark px-3 py-2">
              {filteredComptes.length} résultat{filteredComptes.length > 1 ? 's' : ''}
            </Badge>
          </div>
        </Card.Header>
            ...
            <Table className="table-hover align-middle mb-0">
              <thead className="table-light">
                <tr>
                  ...
                  <th className="border-0 fw-semibold text-muted py-3 px-4">
                    <i className="bi bi-building me-2"></i>Banque
                  </th>
                  <th className="border-0 fw-semibold text-muted py-3 px-4">
                    <i className="bi bi-credit-card me-2"></i>RIB
                  </th>
                  <th className="border-0 fw-semibold text-muted py-3 px-4 text-center">
                    <i className="bi bi-arrow-left-right me-2"></i>Mode Paiement
                  </th>
                  <th className="border-0 fw-semibold text-muted py-3 px-4">
                  ...
                  <tr key={c.id} className="border-bottom">
                    ...
                    <td className="py-3 px-4">
                      <div className="d-flex align-items-center">
                        ...
                        <div>
                          <div className="fw-semibold">{c.banque || "N/A"}</div>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4 text-center">
                      <Badge 
                        bg={c.reglement?.modePaiement === "virement" ? "success" : "info"}
                        className="px-3 py-2"
                      >
                        <i className={`bi ${c.reglement?.modePaiement === "virement" ? "bi-arrow-left-right" : "bi-cash"} me-1`}></i>
                  ...
                  <tr>
                    <td colSpan="9" className="text-center py-5">
                      ...
                      <h5 className="text-muted mb-2">
                        {searchTerm || bankFilter || modeFilter 
                          ? "Aucun règlement trouvé" 
                          : "Aucun règlement bancaire"
                        }
                      </h5>
                      <p className="text-muted mb-3">
                        {searchTerm || bankFilter || modeFilter 
                          ? "Aucun règlement ne correspond à vos critères de recherche"
                          : "Commencez par ajouter votre premier règlement bancaire"
                        }
                      </p>
                      <Button
                        variant="primary"
      ...
      <Modal show={showModal} onHide={handleCloseModal} size="lg" centered>
        <Modal.Header className="bg-primary bg-opacity-10 border-0">
          <Modal.Title className="fw-semibold text-primary">
            <i className="bi bi-bank me-2"></i>
            {formData.id ? "Modifier le Règlement Bancaire" : "Nouveau Règlement Bancaire"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-4">
          <Form onSubmit={handleSubmit}>
            <Row className="g-3">
              <Col md={6}>
                <Form.Group>
                  ...
                  >
                    ...
                    {Array.isArray(factures) && factures.map(f => (
              ...
              <Col md={6}>
                <Form.Group>
                  <Form.Label className="fw-semibold text-muted">RIB *</Form.Label>
                  <InputGroup>
                    <InputGroup.Text className="bg-white border-end-0">
                    ...
                    <Form.Control
                      ...
                      required
                      className="border-start-0 shadow-sm font-monospace"
